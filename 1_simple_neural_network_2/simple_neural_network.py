...
class SimpleNeuralNetwork:
    ...
    def train(self, training_inputs, training_outputs, epochs=5000):
        ...
        for epoch in range(epochs):
            ...
            for i, (x, expected_y) in enumerate(zip(training_inputs, normalized_outputs)):
                ...
                if i == len(training_inputs) - 1:
                    print(f"\rEpoch {epoch + 1:>4}, "
                          f"Error: {total_error / len(training_inputs):>6.4f}, "
                          f"Weight: {self.weights[0]:>6.3f}, "
                          f"Bias: {self.bias[0]:>6.3f}", end='', flush=True)
                    time.sleep(0.001)
